...
def split_train_kshot(train_valid: pd.DataFrame,
    ...
    Strict mode (flex_query=False):
        Keep a user only if n_items >= k_support + k_query; otherwise skip the user.
    Flexible mode (flex_query=True, default):
        - Ensure query has at least min_query rows.
        - If n_items >= k_support + k_query: split normally into (k_support, k_query)
        - If n_items <  k_support + k_query and n_items >= k_support + min_query:
              support = k_support, query = n_items - k_support
        - If n_items <  k_support + min_query and n_items >= min_query + 1:
              support = n_items - min_query, query = min_query
        - If n_items <  min_query + 1: skip the user
    """
    sup_rows, qry_rows = [], []
    skipped = 0
    ...
    for uid, g in train_valid.groupby("user_id", sort=False):
        ...
        if not flex_query:
            # Original strict policy: skip if there are not enough rows
            if n < (k_support + k_query):
                skipped += 1
                continue
            sup_cnt, qry_cnt = k_support, k_query
        else:
            # Flexible policy: try to keep the user; ensure query has at least min_query rows
            if n < (min_query + 1):
                skipped += 1
                continue
            if n >= (k_support + k_query):
                ...
                # n < k_support + min_query and n >= min_query + 1
                qry_cnt = min_query
                sup_cnt = n - qry_cnt
...
def main():
    ...
    ap.add_argument("--flex_query", action="store_true", default=True,
                help="Enable flexible mode: if a user has fewer than K_support+K_query rows, try to keep the user and ensure query has at least min_query rows (enabled by default).")
    ap.add_argument("--min_query", type=int, default=1,
                    help="In flexible mode, the minimum number of rows kept for query (default: 1).")
    ...
    # TRAIN: K-shot split
    # Previously: train_sup, train_qry = split_train_kshot(train_valid, args.k_support, args.k_query, args.seed)
    ...
    assert kept_users + int(skipped) == total_users  # simple consistency check
    ...
    cfg = {
        ...
        "users": {
            ...
            "skipped_users": int(skipped),  # new: skipped users
            ...
            "train_skipped_users": int(skipped),  # new: number of skipped users in training split
        }
    }
